...
class Inertializer():
    ...
    def inertialize_pose_update(
        ...
        world_rotation = (
                    R.from_quat(transition_dst_rotation) * R.from_quat(transition_src_rotation).inv() * R.from_quat(
                bone_input_rotations[0])).as_quat()
        ...
        for i in range(1, len(bone_positions)):
            ...
            bone_rotations[i], bone_avels[i], offset_rotations[i], offset_avels[
                i] = Inertializer.inertialize_update_quat(
                bone_rotations[i],
                bone_avels[i],
                offset_rotations[i],
                offset_avels[i],
...
class CharacterController():
    ...
    def extract_feature_vector(self):
        ...
        # 左脚相对根节点位移
        # 右脚相对根节点位移
        # 根节点速度（由动画数据里当前帧和上一帧的位移除以时间也就是1/60秒得出）
        # 左脚节点速度
        # 右脚节点速度】
        # 所以的量均需要左乘根节点旋转矩阵的逆以将全局量转为局部量
        # 相对位移和旋转只需要投影到地面的向量
        ...
        for i in range(motion.motion_length):
            ...
            if i < motion.motion_length - 60:
                ...
                cur_orientation_inv.apply(joint_translation[i + 20, 0, :] - joint_translation[i, 0, :])[[0, 2]]
                feature_vectors[i, 2:4] = \
                cur_orientation_inv.apply(joint_translation[i + 40, 0, :] - joint_translation[i, 0, :])[[0, 2]]
                feature_vectors[i, 4:6] = \
                cur_orientation_inv.apply(joint_translation[i + 60, 0, :] - joint_translation[i, 0, :])[[0, 2]]
                # 6D future trajectory facing direction
                feature_vectors[i, 6:  8] = \
                cur_orientation_inv.apply(R.from_quat(joint_orientation[i + 20, 0, :]).apply(np.array([0., 0., 1.])))[
                    [0, 2]]
                feature_vectors[i, 8: 10] = \
                cur_orientation_inv.apply(R.from_quat(joint_orientation[i + 40, 0, :]).apply(np.array([0., 0., 1.])))[
                    [0, 2]]
                feature_vectors[i, 10:12] = \
                cur_orientation_inv.apply(R.from_quat(joint_orientation[i + 60, 0, :]).apply(np.array([0., 0., 1.])))[
                    [0, 2]]
            # 6D foot local position
            feature_vectors[i, 12:15] = cur_orientation_inv.apply(
                joint_translation[i, joint_name.index('rAnkle'), :] - joint_translation[i, 0, :])
            feature_vectors[i, 15:18] = cur_orientation_inv.apply(
                joint_translation[i, joint_name.index('lAnkle'), :] - joint_translation[i, 0, :])

            if i != 0:
                delta_time = self.dt
                # 3D hip local velcity
                feature_vectors[i, 18:21] = cur_orientation_inv.apply( \
                    (joint_translation[i, joint_name.index('RootJoint'), :] - joint_translation[i - 1,
                                                                              joint_name.index('RootJoint'),
                                                                              :]) / delta_time)
                # 6D foot local velcity
                hip_velocity = feature_vectors[i, 18:21]
                feature_vectors[i, 21:24] = cur_orientation_inv.apply( \
                    (joint_translation[i, joint_name.index('rAnkle'), :] - joint_translation[i - 1,
                                                                           joint_name.index('rAnkle'),
                                                                           :]) / delta_time) - hip_velocity
                feature_vectors[i, 24:27] = cur_orientation_inv.apply( \
                    (joint_translation[i, joint_name.index('lAnkle'), :] - joint_translation[i - 1,
                                                                           joint_name.index('lAnkle'),
                                                                           :]) / delta_time) - hip_velocity
    ...
    def update_state(self,
        ...
        cur_feature_vector[6:  8] = \
        root_orientation_inv.apply(R.from_quat(desired_rot_list[1]).apply(np.array([0., 0., 1.])))[[0, 2]]
        cur_feature_vector[8: 10] = \
        root_orientation_inv.apply(R.from_quat(desired_rot_list[2]).apply(np.array([0., 0., 1.])))[[0, 2]]
        cur_feature_vector[10:12] = \
        root_orientation_inv.apply(R.from_quat(desired_rot_list[3]).apply(np.array([0., 0., 1.])))[[0, 2]]
        # normalize tarjectory feature
        cur_feature_vector[:12] = np.divide(cur_feature_vector[:12] - self.features_offset[:12],
                                            self.features_scale[:12])
        ...
        if best_frame != self.cur_frame and search:
            ...
            Inertializer.inertialize_pose_transition(
                ...
                self.offset_avels,
                # set src and dst for inertialize（函数中设置）
